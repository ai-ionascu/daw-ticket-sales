...
{   
    if(empty($_POST['email'])){
        ...
        $email_existing_query = 'SELECT email FROM users WHERE email="$email" LIMIT 1';
...
if(isset($_POST['reg_btn']))
{
    if(empty($_POST['username'])){
        ...
        $username_existing_query = 'SELECT email FROM users WHERE username="$username" LIMIT 1';
    ...
    if(!empty($_SESSION['error'])){
        ...
        $register_query = "INSERT INTO users (username, email, password) 
